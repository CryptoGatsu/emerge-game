...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (judge) {
      const secret = process.env.EMERGE_CRON_SECRET;
      if (!secret || request.headers.get('x-cron-secret') !== secret) return NextResponse.json({ error: 'Not here.' }, { status: 404 });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === 'age') {
      // A test hook: age a claim past its shield. Only with the cron secret,
      // and never where the token is live.
      const secret = process.env.EMERGE_CRON_SECRET;
      if (tokenLive() || !secret || request.headers.get('x-cron-secret') !== secret) return NextResponse.json({ error: 'Not here.' }, { status: 404 });
