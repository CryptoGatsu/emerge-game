...
export async function invade(targetSeed: number, attacker: string, fromSeed: number, troops: number): Promise<WarResult<{ claim: Claim; battle: Battle; home: Claim }>> {
  ...
  return withLock(async () => {
    ...
    // War is entered, not suffered. A plot with no base has no troops and no
    // way to get any, so an invasion of one is not a fight: it is a stranger
    // taking three fifths of a city's yield, unopposed, for as long as they
    // care to pay the keep. Every plot claimed before this release is in
    // exactly that position. So a plot is a target once it has opened a base
    // of its own — the same 250,000 $EMERGE everybody else paid — and an
    // occupied plot stays one, because the garrison is there to be fought.
    if (!target.army && !target.occupation) {
      return { ok: false, reason: `${target.worldName || 'That plot'} has no base, so it is not at war. Only plots that have opened one can be marched on.`, status: 409 };
    }
