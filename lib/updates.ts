...
export const UPDATES: Update[] = [
  {
    ...
    notes: [
      ...
      'An age is a step up for the whole town. Every workplace takes one more person per age the plot has reached, every pair of hands makes 12% more, the improvement cap rises a level, and the Build panel opens on the age’s own buildings with the earlier ages a tab away and the later ones there to look at. Players said advancing felt like unlocking a few buildings; it is now the town growing.',
...
export const UPDATES_ZH: Update[] = [
  {
    ...
    notes: [
      ...
      '时代是整座城的升级。地块每推进一个时代，每个工作场所多容纳一人，每双手多产出 12%，升级上限提高一级，建造面板默认打开本时代的建筑，早期时代在旁边的标签里，后面的时代也能先看。玩家说推进时代像只是解锁几栋建筑；现在它是整座城在成长。',
