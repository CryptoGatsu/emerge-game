...
export const UPDATES: Update[] = [
  {
    ...
    notes: [
      ...
      'War. A plot can open a military base for 250,000 $EMERGE, burned like every charge; a barracks is raised by the square and becomes a garrison, an armoury, a base and a drone bay with the ages. Troops are trained there in Gold — 300 a militiaman in a settlement, up to 1,600 a drone in the AI age, plus steel from the industrial age — twelve a day at most, thirty to a hundred and ten to a base by age. Every age fights differently: militia are cheap and strong behind their own fences, men-at-arms the best defence for the price, riflemen hit hard away and stand thin at home, the armoured corps is strong both ways, the drone corps has the hardest attack and the least to fall back on at home.',
      'Invasion and occupation. From the world map, march your army on any plot somebody else holds; the card shows the odds. The registry fights the battle on the server, on numbers everybody can see and a secret it commits to first, and both settlements play the same fight back on the ground: the attackers come in from the gate in bounds, the garrison takes cover beside the buildings round the square, spears and rifles are aimed and fired with every round, the ones caught in the open fall first, and the townsfolk run home and bar the doors until it is over; afterwards the patrols walk the buildings they hold and go in and out of them. Win, and your army holds the plot: its soldiers patrol the buildings under your banner, 60% of the plot’s daily yield comes to you, and it costs your treasury 250 Gold a day and up to keep them there. The owner throws them out with their own army, fighting with the home advantage; anybody else can ambush the garrison and take the plot over. An army holds one plot at a time. A plot cannot be invaded for 24 hours after it is claimed, or after its owner retakes it. Every invasion, every defence and every retake is announced on every screen, and the world map marks every plot under siege.',
    ],
  },
  {
    version: '2.6',
